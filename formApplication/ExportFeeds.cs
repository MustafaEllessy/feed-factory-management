...
{
    public partial class ExportFeeds : Form
    {
        ...
        private void frmExportedFeed_Load(object sender, EventArgs e)
        {
            loadFeedStore();
        }
        private void loadFeedStore()
        {
            ...
            for (int i = 0; i < dtFeedStore.Rows.Count; i++)
            {
                ...
                dgvExportedFeeds.Rows.Add(new object[] { i + 1, feedName, 0, 0 });
        ...
        {
            int takenValue = (Convert.ToInt32(dgvExportedFeeds.Rows[e.RowIndex].Cells[2].Value.ToString()) * 1000) + (Convert.ToInt32(dgvExportedFeeds.Rows[e.RowIndex].Cells[3].Value.ToString()) * 1000000);
            if (takenValue > 0)
            {
        ...
        private void dgvExportedFeeds_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (takenValue < 0)
                {
                    dgvExportedFeeds.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = 0;
                    return;
                }
        ...
        public void addProducedFeed(DataGridViewRow dr)
        {
            ...
            object[] rawData = new object[] { exportDate, feedName, gram, "جرام",cbxStationName.SelectedItem.ToString(),cbxStationType.SelectedItem.ToString()};
            DB.insertToDB("exportedFeed", new string[] { "exportDate", "producedFeed", "quantity", "unit", "stationName", "stationType" }, rawData);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("املأ الخانات الفارغة");
                    return;
                }
                List<int> row = rowIndex.Distinct().ToList();
                for (int i = 0; i < row.Count; i++)
                {
                    DataGridViewRow dr = dgvExportedFeeds.Rows[row[i]];
                ...
                for (int i = 0; i < row.Count; i++)
                {
                    dgvExportedFeeds.Rows[row[i]].Cells[2].Value=0;
                    dgvExportedFeeds.Rows[row[i]].Cells[3].Value = 0;
                }
                row.Clear();
